...
export function createAnimations(scene) {
    ...
    // Personaje 3 - Animación de caminata
    if (scene.textures.exists('personaje3_walk')) {
        try {
            const total = scene.textures.get('personaje3_walk').frameTotal;
            const end = Math.max(0, total - 1);
            if (!scene.anims.exists('personaje3_walk')) {
                scene.anims.create({
                    key: 'personaje3_walk',
                    frames: scene.anims.generateFrameNumbers('personaje3_walk', { start: 0, end }),
                    frameRate: 10,
                    repeat: -1
                });
            }
        } catch (e) {
            console.warn('No se pudo crear animación personaje3_walk:', e);
        }
    }
}
