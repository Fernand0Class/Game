...
class PreloadScene extends Phaser.Scene {
    ...
    create() {
        ...
        spawnEnemy(this, this.enemies, 200, 300);
        spawnEnemy(this, this.enemies, 600, 250);
        spawnEnemy(this, this.enemies, 400, 450);
        
        addEnemyPlatformCollisions(this, this.enemies, platforms);
        addPlayerEnemyCollisions(this, this.player, this.enemies, (enemy) => {
            this.loseLife();
        });
            } catch (e) {
    ...
    create() {
        ...
        if (this.game && this.game.canvas) {
            ...
            this.game.canvas.focus();
                ...
                // Controles de ataque smash
        this.input.keyboard.on('keydown-X', () => {
            if (canAttack(this.player)) {
                const hitSomething = executeSmashAttack(this.player, this, this.enemies);
                if (hitSomething) {
                    setAttackCooldown(this.player);
                    // Opcional: agregar efecto visual/sonido de ataque
                    this.player.setTint(0xff00ff);
                    setTimeout(() => this.player.setTint(this.player.tint), 100);
                }
            }
        });
        ...
        this.add.text(20, height - 40, 'Flechas: mover | Espacio: saltar', { fontSize: '14px', color: '#cccccc' }).setOrigin(0, 0);
                this.add.text(20, 20, 'ESC: Menú | X o Z: Ataque | Flechas: Mover | Space: Saltar', {
            fontSize: '12px',
            color: '#ffffff'
        });
...
class PlayScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(20, 20, 'Presiona ESC para volver al menú', { fontSize: '14px', color: '#ffffff' });
                ...
                // ========== ACTUALIZAR ENEMIGOS ==========
        updateEnemies(this, this.enemies, this.player, this.platforms);
        const music = this.registry.get('bgm');
        if (music && !music.isPlaying) music.play();
        if (music) {
